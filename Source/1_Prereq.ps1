...
'OS','Tools\DSC','Tools\ToolsVHD\DiskSpd' | ForEach-Object {
    if (!( Test-Path "$Workdir\$_" )) { New-Item -Type Directory -Path "$workdir\$_" } }

If (!( Test-Path -Path "$workdir\OS\Copy_WindowsServer_ISO_here.txt" )) { 
	   New-Item -Path "$workdir\OS\" -Name Copy_WindowsServer_ISO_here.txt -ItemType File }
...
$modules=('xActiveDirectory',"2.9.0.0"),('xDHCpServer',"1.3.0.0"),('xNetworking',"2.7.0.0")
